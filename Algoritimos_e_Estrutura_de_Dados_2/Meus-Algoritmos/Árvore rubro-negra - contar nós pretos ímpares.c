...
int conta_pretos_impar(NodeVP *tree)
{
    if (tree != NULL)
    {
        return (tree->color == 1 && tree->key % 2 != 0) + conta_pretos_impar(tree->left) + conta_pretos_impar(tree->right);
    }
...
int main()
{
    ...
    printf("%d", conta_pretos_impar(arvore));
